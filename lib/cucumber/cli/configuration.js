      ...
      for (var i = 0; i < formats.length; i++) {
        ...
        switch (format) {
          case Configuration.JSON_FORMAT_NAME:
            formatters.push(Cucumber.Listener.JsonFormatter(options));
            break;
          case Configuration.PROGRESS_FORMAT_NAME:
            formatters.push(Cucumber.Listener.ProgressFormatter(options));
            break;
          case Configuration.PRETTY_FORMAT_NAME:
            formatters.push(Cucumber.Listener.PrettyFormatter(options));
            break;
          case Configuration.SUMMARY_FORMAT_NAME:
            formatters.push(Cucumber.Listener.SummaryFormatter(options));
            break;
          default:
